...
| Automated Critical Regression Valid | Backend and PostgreSQL runtime; Browser / chromium-regression; all inherited CI jobs | PostgreSQL and inherited browser/device profiles | `@regression-booking`, `@regression-lifecycle`, `@regression-crm-history`, `@regression-pet-documents`, `@regression-checkout`, `@regression-calendar-time`, `@regression-scheduling-replay` | Accessibility, remaining mandatory pilot findings, staging, manual UX, and physical devices remain separate gates |
...
| Scheduling Durable Replay Protection Valid | Backend and PostgreSQL runtime; Browser / chromium-regression; all inherited CI jobs | PostgreSQL transaction/concurrency integration and desktop Chromium | Database scheduling replay regression; `@regression-scheduling-replay` | Protects create/reschedule only; pilot retains replay rows without automatic cleanup; cancellation/lifecycle replay remains outside E3 scope |

...
function renderAgendaCalendar(){
  ...
  target.className="calendar-agenda";target.style.removeProperty("min-width");target.style.removeProperty("--groomer-count");target.innerHTML=items.length?items.map(item=>{const groomers=(item.groomers||[]).map(g=>g.displayName).join(", ")||item.employeeName,services=item.services.map(service=>service.name).join(", ");return `<section class="agenda-entry"><time datetime="${escape(item.startAt)}"><strong>${new Intl.DateTimeFormat([],{weekday:"long",month:"short",day:"numeric",timeZone:item.schedulingTimezone||schedulingZone()}).format(new Date(item.startAt))}</strong><span>${schedulingTime(item)}</span></time><button type="button" class="agenda-appointment" data-calendar-appointment="${item.id}" aria-label="Open ${escape(item.petName)} appointment for ${escape(item.firstName)} ${escape(item.lastName)}"><strong>${escape(item.petName)}</strong><span>${escape(item.firstName)} ${escape(item.lastName)}</span><span>${escape(services)}</span><small>${escape(groomers)} · ${escape(item.status.replace("_"," "))}</small></button>${safetyContext(item)}</section>`;}).join(""):"<p class=\"empty\">No appointments in this period.</p>";

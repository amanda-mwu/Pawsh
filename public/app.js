...
function renderCalendar(){if(state.calendar.view==="month")renderMonthCalendar();else if(state.calendar.view==="day")renderDayCalendar();else renderWeekCalendar();}
...
function renderMonthNavigator(){
  if(!state.calendar.month)return;const first=`${state.calendar.month}-01`,start=dateShift(first,-dateAt(first).getUTCDay());const appointmentDates=new Set(filteredAppointments(state.calendar.monthAppointments).map(item=>appointmentLocalValue(item).slice(0,10)));
...
async function loadCalendarMonth(month=state.calendar.month,render=true){const first=`${month}-01`,start=dateShift(first,-dateAt(first).getUTCDay()),appointments=await loadAppointmentRange(start,42);state.calendar.monthAppointments=appointments;if(render)renderMonthNavigator();return appointments;}
...
function updateCalendarViewControls(){for(const view of ["month","week","day"])$(`#calendar-${view}-view`).setAttribute("aria-pressed",String(state.calendar.view===view));}
function setCalendarView(view){state.calendar.view=view;updateCalendarViewControls();loadCalendarWeek();}
$("#calendar-month-view").addEventListener("click",()=>setCalendarView("month"));$("#calendar-week-view").addEventListener("click",()=>setCalendarView("week"));$("#calendar-day-view").addEventListener("click",()=>setCalendarView("day"));
$("#month-prev").addEventListener("click",async()=>{const first=dateAt(`${state.calendar.month}-01`);first.setUTCMonth(first.getUTCMonth()-1);state.calendar.month=first.toISOString().slice(0,7);if(state.calendar.view==="month")await loadCalendarWeek();else await loadCalendarMonth();});$("#month-next").addEventListener("click",async()=>{const first=dateAt(`${state.calendar.month}-01`);first.setUTCMonth(first.getUTCMonth()+1);state.calendar.month=first.toISOString().slice(0,7);if(state.calendar.view==="month")await loadCalendarWeek();else await loadCalendarMonth();});

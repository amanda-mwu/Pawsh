...
function openCalendarAppointment(id){const item=state.appointments.find(appointment=>appointment.id===id);if(!item)return;const groomers=(item.groomers||[]).map(groomer=>groomer.displayName).join(", ")||item.employeeName;openModal(`${item.petName} appointment`,`<div class="wide"><p><strong>${escape(item.firstName)} ${escape(item.lastName)}</strong> · ${escape(groomers)}</p><p>${escape(item.services.map(service=>service.name).join(", "))}</p><p>${new Intl.DateTimeFormat([],{dateStyle:"full",timeStyle:"short",timeZone:item.schedulingTimezone||schedulingZone()}).format(new Date(item.startAt))}</p>${safetyContext(item)}${item.status==="scheduled"&&allowed("appointments.edit")?`<button type="button" class="secondary calendar-move-detail">Move appointment</button>`:""}</div>`,async()=>{});$(".calendar-move-detail")?.addEventListener("click",()=>{$("#modal").close();moveAppointment(id);});}

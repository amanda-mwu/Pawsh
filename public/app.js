...
async function bootstrap() {
  try {
    ...
    const initialView=viewForPath(location.pathname);activateView(initialView,{history:"replace"});if(initialView==="admin-settings")renderSettingsCategory(settingsPathCategory(),{history:"replace"});
...
function viewForPath(path){if(path==="/account")return "profile-account";if(path==="/settings"||path.startsWith("/settings/"))return "admin-settings";return path==="/salon/breeds"||path==="/reports/breeds"||path==="/overview/breeds"?"breed-catalog":"dashboard";}
...
function activateView(view,{history="push"}={}) {
  ...
  const canonicalPath=viewPaths[view];if(canonicalPath&&history!=="none"&&view!=="admin-settings"&&(location.pathname!==canonicalPath||view==="breed-catalog")){globalThis.history[history==="replace"?"replaceState":"pushState"]({view},"",canonicalPath);}
...
function printableAgenda(items){const sorted=items.slice().sort((a,b)=>new Date(a.startAt)-new Date(b.startAt));return sorted.length?sorted.map(item=>{const model=appointmentPresentation(item);return `<article class="print-appointment"><header><strong>${escape(model.groomers.join(", "))}</strong><span>${escape(model.dateLabel)} · ${escape(model.timeRange)}</span></header><div><p><b>Pet:</b> ${escape(model.petName)}${model.breed?` · ${escape(model.breed)}`:""}</p><p><b>Services:</b> ${model.services.map(escape).join(", ")}</p><p><b>Client:</b> ${escape(model.customerName)}${item.customerPhone?` · ${escape(item.customerPhone)}`:""}</p>${item.notes?`<p><b>Appointment note:</b> ${escape(item.notes)}</p>`:""}</div></article>`;}).join(""):`<p>No appointments in this print range.</p>`;}

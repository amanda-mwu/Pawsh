...
function appointmentPresentation(item){
  const start=new Date(item.startAt),end=new Date(item.endAt),zone=item.schedulingTimezone||schedulingZone(),formatTime=value=>new Intl.DateTimeFormat([],{hour:"numeric",minute:"2-digit",timeZone:zone}).format(value),services=item.services.map(service=>service.name),groomers=(item.groomers||[]).map(groomer=>groomer.displayName);
  return {id:item.id,date:appointmentLocalValue(item).slice(0,10),dateLabel:new Intl.DateTimeFormat([],{weekday:"long",month:"long",day:"numeric",timeZone:zone}).format(start),timeRange:`${formatTime(start)}–${formatTime(end)}`,petName:item.petName,breed:item.breed||"",customerName:`${item.firstName} ${item.lastName}`,services,groomers:groomers.length?groomers:[item.employeeName],status:item.status.replace("_"," "),conflictOverridden:Boolean(item.conflictOverridden),rabiesNeeded:["not_provided","expires_before_appointment"].includes(item.rabiesAppointmentStatus),warning:item.safetyAlerts||item.behaviorNotes||item.medicalNotes||item.groomingPreferences||item.coatNotes||"",durationMinutes:Math.max(1,Math.round((end-start)/60000))};
}
...
function appointmentHoverDetails(model,id=""){return `<aside ${id?`id="${escape(id)}"`:""} class="appointment-hover" role="tooltip"><div><span>Status</span><strong>${escape(model.status)}</strong></div><p><strong>${escape(model.dateLabel)}</strong><br>${escape(model.timeRange)}</p><dl><div><dt>Customer</dt><dd>${escape(model.customerName)}</dd></div><div><dt>Pet</dt><dd>${escape(model.petName)}${model.breed?` · ${escape(model.breed)}`:""}</dd></div><div><dt>Services</dt><dd>${model.services.map(escape).join("<br>")}</dd></div><div><dt>Groomer</dt><dd>${model.groomers.map(escape).join(", ")}</dd></div></dl>${model.rabiesNeeded?`<p class="hover-warning">Rabies needed</p>`:""}${model.warning?`<p class="hover-warning">⚠ ${escape(model.warning)}</p>`:""}</aside>`;}
...
function appointmentCard(item,{day=false,style="",groomerId="",overlap=false}={}){
  const model=appointmentPresentation(item),density=model.durationMinutes<=30?"short":model.durationMinutes<90?"medium":"long",shown=model.services.slice(0,density==="long"?3:1),extra=model.services.length-shown.length,multiple=model.groomers.length>1;
  const hoverId=`appointment-hover-${item.id}-${groomerId||"calendar"}`;return `<article class="${day?"day-appointment ":""}week-appointment appointment-block density-${density} status-${escape(item.status)} ${overlap?"overlap":""}" data-appointment-id="${item.id}" ${groomerId?`data-groomer-id="${groomerId}"`:""} style="${style}"><button type="button" class="calendar-open" data-calendar-appointment="${item.id}" aria-describedby="${hoverId}" aria-label="${escape(appointmentAccessibleName(model))}"><time>${escape(model.timeRange)}</time><strong class="appointment-pet">${escape(model.petName)}</strong>${model.breed?`<span class="appointment-breed">${escape(model.breed)}</span>`:""}<span class="appointment-services">${shown.map(escape).join("<br>")}${extra>0?`<small>+${extra} more</small>`:""}</span>${multiple?`<small class="multi-groomer">${model.groomers.length} groomers</small>`:""}<span class="appointment-badges"><small class="appointment-status">${escape(model.status)}</small>${model.conflictOverridden?`<small class="conflict-override" data-testid="conflict-override">Intentional overlap</small>`:""}${model.rabiesNeeded?`<small class="card-warning" aria-label="Rabies needed">!</small>`:""}${model.warning?`<small class="card-warning" aria-label="Pet warning">⚠</small>`:""}</span></button><div class="sr-only appointment-accessible-safety">${safetyContext(item)}</div><div class="appointment-quick-actions">${calendarAction(item)}</div>${appointmentHoverDetails(model,hoverId)}</article>`;
}
...
function renderAgendaCalendar(){
  ...
  const groups=items.reduce((map,item)=>{const date=appointmentPresentation(item).date,mapItems=map.get(date)||[];mapItems.push(item);map.set(date,mapItems);return map;},new Map());target.className="calendar-agenda";target.style.removeProperty("min-width");target.style.removeProperty("--groomer-count");target.innerHTML=items.length?[...groups].map(([date,group])=>`<section class="agenda-day"><h3>${new Intl.DateTimeFormat([],{weekday:"long",month:"long",day:"numeric"}).format(dateAt(date))}</h3>${group.map(item=>{const model=appointmentPresentation(item);return `<article class="agenda-entry"><time datetime="${escape(item.startAt)}">${escape(model.timeRange)}</time><button type="button" class="agenda-appointment" data-calendar-appointment="${item.id}" aria-label="${escape(appointmentAccessibleName(model))}"><strong>${escape(model.petName)}${model.breed?` <span>(${escape(model.breed)})</span>`:""}</strong><span>${escape(model.customerName)}</span><span>${model.services.map(escape).join(", ")}</span><small>${model.groomers.map(escape).join(", ")}</small></button><div class="agenda-indicators"><span class="appointment-status">${escape(model.status)}</span>${model.rabiesNeeded?`<span class="rabies-needed">Rabies needed</span>`:""}${model.warning?`<span class="agenda-warning">⚠ ${escape(model.warning)}</span>`:""}</div>${appointmentHoverDetails(model)}</article>`;}).join("")}</section>`).join(""):"<p class=\"empty\">No appointments in this period.</p>";
...
function renderMonthCalendar(){
  ...
  const first=`${state.calendar.month}-01`,start=dateShift(first,-dateAt(first).getUTCDay()),days=Array.from({length:42},(_,index)=>dateShift(start,index)),today=businessDate(),visible=filteredAppointments(state.calendar.monthAppointments.length?state.calendar.monthAppointments:state.appointments);
...
async function loadCalendarMonth(month=state.calendar.month){const first=`${month}-01`,start=dateShift(first,-dateAt(first).getUTCDay()),appointments=await loadAppointmentRange(start,42);state.calendar.monthAppointments=appointments;return appointments;}
...
const viewPaths={dashboard:"/",calendar:"/",customers:"/",messages:"/",reminders:"/","sales-expense":"/",product:"/",services:"/",setup:"/","breed-catalog":"/salon/breeds","admin-settings":"/settings",reports:"/","profile-account":"/account"};
...
$("#calendar-today").addEventListener("click",()=>selectCalendarDate(businessDate()));$("#calendar-prev-week").addEventListener("click",()=>calendarStep(-1));$("#calendar-next-week").addEventListener("click",()=>calendarStep(1));
...
function setCalendarView(view){state.calendar.view=view;state.calendar.displayMode="calendar";updateCalendarViewControls();loadCalendarWeek();}

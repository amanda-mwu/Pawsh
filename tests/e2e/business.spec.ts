...
test("@smoke the lock takes the move affordance away and says why",async({page,request,tenant})=>{
  ...
  // The detail dialog is the other place Move is offered, and it answers the same question.
  await page.keyboard.press("Escape");
  await locked.locator(".calendar-open").click();
  await expect(page.getByTestId("modal")).toBeVisible();
  await expect(page.locator(".appointment-detail-move")).toHaveCount(0);
  await expect(page.getByTestId("appointment-detail-lock-note"))
    .toContainText("Appointments are locked from being moved.");
  // Its neighbours in the same footer are untouched.
  await expect(page.getByRole("button",{name:"View client"})).toBeVisible();
  await page.getByTestId("modal").getByRole("button",{name:"Close appointment details"}).click();
